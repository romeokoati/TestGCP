...
class UniformQuickFill:
    ...
    def ExecuteBaseCaseSubStr(self,elt,sigma):
        ...
        if s11[0].startswith("Cpos"):
            ...
            if p1[1].rfind('TokenSeq') >=0:
                ...
                p1 = self.Pos(sigma[s1[0]],regexpression1,regexpression2,int(p11[2]))  # indice de debut
        ...
        if p2 == self.BOTTOM or p1 == self.BOTTOM or p2 > len(sigma[s1[0]]):
            return self.BOTTOM
    ...
    def ExecuteElement(self,sigma,TraceExpression):
        ...
        for elt in AtomiqueExpression:
            if elt.startswith("Loop"):
                y = self.ExecuteBaseCaseLoop(AtomiqueExpression,sigma)
                Result.append(y)
                break
                
            elif elt.startswith("SubStr("):
