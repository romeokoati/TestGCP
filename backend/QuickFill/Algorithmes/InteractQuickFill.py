...
class InteractQuickFill:
    ...
    def GenerateSubstring(self,entree,s,bi):
        ...
        """ On va retourner les expressions SubStr avec uniquement les expressions Pos, car elles sont basee sur les expressions regulieres """
        result = set()
        if s in entree:     
            k = entree.index(s)         
            k1 = len(s)+k-1
...
######################### Eriture de la fonction des programmes
    ...
    def ExecuteBaseCaseSubStr(self,elt,sigma):
        ...
        if p1[1].rfind('TokenSeq') >=0:
            ...
            p1 = self.Pos(sigma,regexpression1,regexpression2,int(p11[2]))  # indice de debut
        ...
        if p2[1].rfind('TokenSeq') >=0:
            ...
            p2 = self.Pos(sigma,regexpression1,regexpression2,int(p22[2]))
        ...
        if p2 == self.BOTTOM or p1 == self.BOTTOM or p2 > len(sigma):
            return self.BOTTOM

        else:
            return self.SubStr(sigma,p1,p2)
    ...
    def ExecuteFonction(self,TraceExpression,entree,valeur):
        ...
        for elt in AtomiqueExpression:
            if elt.startswith("SubStr("):
                t = str(elt).split(",",1)
                t = t[0][len(t[0])-2:]
                y = self.ExecuteBaseCaseSubStr(elt,sigma[t])
                Result.append(y)

            elif elt.startswith("SubStr2"):
                y = self.ExecuteBaseCaseSubstr2(elt,valeur)
                Result.append(y)
